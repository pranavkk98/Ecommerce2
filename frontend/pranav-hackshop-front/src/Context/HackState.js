...
import Context from "./HackContext";
import reducer from "./HackReducer";
...
const HackState = (props) => {
  ...
  const setAlert = (open, payload, severity) => {
    dispatch({
      type: SET_ALERT,
      payload: {
        open,
  ...
  const compareCartToOrder = () => {
    if (state.cart.cart.cartItems.length !== state.order.products.length) {
      return false;
    }
    for (let i = 0; i < state.cart.cart.cartItems.length; i++) {
      if (
        state.cart.cart.cartItems[i].product !==
          state.order.products[i].product._id ||
        state.cart.cart.cartItems[i].quantity !==
          state.order.products[i].quantity ||
        state.cart.cart.cartItems[i].totalPrice !==
          state.order.products[i].amount
      ) {
        return false;
      }
    }
  ...
  const getUserInfo = async () => {
    try {
      ...
      dispatch({ type: SET_TOKEN, payload: localStorage.getItem("jwt") });
    } catch (error) {
      console.error(error);
      dispatch({ type: SET_LOADING, payload: false });
      SetAuthToken();
  ...
  const registerUser = async (name, email, password, confirmPassword) => {
    try {
      ...
      dispatch({ type: SET_LOADING, payload: false });
      setAlert(true, "User Registered. Confirmation mail sent.", "success");
      return true;
    } catch (error) {
      dispatch({ type: SET_LOADING, payload: false });
      setAlert(
  ...
  const getCategories = async () => {
    try {
      ...
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}public/getCategories`
      );
      dispatch({
        type: SET_CATEGORIES,
  ...
  const getProducts = (categoryName) => {
    ...
    if (categoryName !== "All") {
      ...
      for (let i in allProducts) {
        let objId = allProducts[i]._id;
        uniqueObject[objId] = allProducts[i];
      }
      for (let j in uniqueObject) {
        products.push(uniqueObject[j]);
      }
    }
    return products;
  };
  ...
  const getProduct = async (id) => {
    try {
      dispatch({ type: SET_LOADING, payload: true });
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}public/getProduct/${id}`
      );
      dispatch({ type: SET_LOADING, payload: false });
      if (!res.data.info.image.length) {
        res.data.info.image[0] = {
          imgUrl:
            "https://i5.walmartimages.com/asr/4add4de6-7b92-4846-8316-b7a0cbec4dc7_1.8e2f7305081b9284e56d112fe146dc90.png",
        };
      }
      dispatch({ type: SET_PRODUCT, payload: res.data.info });
      return true;
    } catch (error) {
      dispatch({ type: SET_LOADING, payload: false });
      setAlert(
  ...
  const getCart = async () => {
    try {
      dispatch({ type: SET_LOADING, payload: true });
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}user/cart/getCart/`
      );
      dispatch({ type: SET_LOADING, payload: false });
      dispatch({ type: SET_CART, payload: res.data.info });
      return true;
    } catch (error) {
      dispatch({ type: SET_LOADING, payload: false });
      setAlert(
  ...
  const getDeliverySpeeds = async () => {
    try {
      dispatch({ type: SET_LOADING, payload: true });
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}user/cart/getDeliverySpeeds/`
      );
      dispatch({ type: SET_LOADING, payload: false });
      dispatch({ type: SET_DELIVERY_SPEEDS, payload: res.data.info });
      return true;
    } catch (error) {
      dispatch({ type: SET_LOADING, payload: false });
      setAlert(
  ...
  const getAddresses = async () => {
    try {
      dispatch({ type: SET_LOADING, payload: true });
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}user/address/getAddresses/`
      );
      dispatch({ type: SET_LOADING, payload: false });
      dispatch({ type: SET_ADDRESSES, payload: res.data.info });
      return true;
    } catch (error) {
      dispatch({ type: SET_LOADING, payload: false });
      setAlert(
  ...
  const getOrder = async (id) => {
    try {
      dispatch({ type: SET_LOADING, payload: true });
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}user/order/getOrder/${id}`
      );
      dispatch({ type: SET_LOADING, payload: false });
      dispatch({ type: SET_ORDER, payload: res.data.info });
      return true;
    } catch (error) {
      dispatch({ type: SET_LOADING, payload: false });
      setAlert(
  ...
  const getYourOrders = async () => {
    try {
      dispatch({ type: SET_LOADING, payload: true });
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}user/order/getYourOrders/`
      );
      dispatch({ type: SET_LOADING, payload: false });
      dispatch({ type: SET_YOUR_ORDERS, payload: res.data.info });
      return true;
    } catch (error) {
      dispatch({ type: SET_LOADING, payload: false });
      setAlert(
  ...
  const addCartItem = async (product, totalPrice) => {
    try {
      dispatch({ type: SET_LOADING, payload: true });
      const itemInCart = state.cart.cart.cartItems.find(
      ...
      if (itemInCart !== undefined) {
        return updateCartItem(
          ...
          itemInCart.totalPrice * (1 + 1 / itemInCart.quantity)
        );
      }
      const res = await axios.post(
        `${process.env.REACT_APP_API_URL}user/cart/add/${state.user._id}/`,
        {
          product,
          quantity: 1,
          totalPrice,
  ...
  const updateCartItem = async (itemId, quantity, totalPrice) => {
    try {
      dispatch({ type: SET_LOADING, payload: true });
      const res = await axios.put(
        `${process.env.REACT_APP_API_URL}user/cart/update/${state.user._id}/`,
        {
          itemId,
          quantity,
          totalPrice,
  ...
  const deleteCartItem = async (itemId) => {
    try {
      ...
      const res = await axios.delete(
        `${process.env.REACT_APP_API_URL}user/cart/delete/${state.user._id}`,
        {
          data: {
            itemId: itemId,
          },
  ...
  const addAddress = async (
    ...
    try {
      dispatch({ type: SET_LOADING, payload: true });
      const res = await axios.post(
        `${process.env.REACT_APP_API_URL}user/address/add`,
        {
          addressLineOne,
          addressLineTwo,
          landmark,
  ...
  const editAddress = async (
    ...
    try {
      dispatch({ type: SET_LOADING, payload: true });
      const res = await axios.put(
        `${process.env.REACT_APP_API_URL}user/address/update/${addressId}`,
        {
          addressLineOne,
          addressLineTwo,
          landmark,
  ...
  const deleteAddress = async (addressId) => {
    try {
      dispatch({ type: SET_LOADING, payload: true });
      const res = await axios.delete(
        `${process.env.REACT_APP_API_URL}user/address/delete/${addressId}`
      );
      dispatch({ type: SET_LOADING, payload: false });
      setAlert(true, res.data.info, "success");
      getAddresses();
  ...
  const generateOrder = async (addressId, deliverySpeedId) => {
    try {
      dispatch({ type: SET_LOADING, payload: true });
      const res = await axios.post(
        `${process.env.REACT_APP_API_URL}user/order/generate/`,
        {
          addressId,
          deliverySpeedId,
        }
  ...
  const deleteOrder = async (orderId) => {
    try {
      dispatch({ type: SET_LOADING, payload: true });
      await axios.delete(
        `${process.env.REACT_APP_API_URL}user/order/deleteOrder/${orderId}`
      );
      dispatch({ type: SET_LOADING, payload: false });
      return true;
    } catch (error) {
  ...
  const deleteDeliverySpeed = async (deliverySpeedId) => {
    try {
      dispatch({ type: SET_LOADING, payload: true });
      await axios.delete(
        `${process.env.REACT_APP_API_URL}admin/deliverySpeed/delete/${deliverySpeedId}`
      );
      dispatch({ type: SET_LOADING, payload: false });
      return true;
    } catch (error) {
  ...
  const verifyToken = async (token) => {
    try {
      setLoading(true);
      await axios.get(
        `${process.env.REACT_APP_API_URL}user/verifyUser/${token}`
      );
      setLoading(false);
      return true;
    } catch (error) {
