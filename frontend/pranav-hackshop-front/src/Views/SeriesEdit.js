...
const CategoryEdit = () => {
  ...
  const updateSeries = async (name, description, seriesId) => {
    try {
      setLoading(false);
      const res = await axios.put(
        `${process.env.REACT_APP_API_URL}admin/series/update/${seriesId}`,
        {
          name,
          description,
        }
  ...
  const handleSubmit = async (name, description) => {
    const res = await updateSeries(name, description, id);
    setRedirect(res);
  };
